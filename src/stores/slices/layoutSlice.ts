...
export function createLayoutSlice(set: SetState, get: GetState): LayoutSlice {
  return {
    updateNodePosition: (nodeId: string, position: XYPosition) => {
      ...
      let updatedNodeMetadata: Record<
        string,
        (typeof metadata.nodeMetadata)[string]
      >
      ...
      if (draggedItem.level === 1) {
        ...
        if (parentRoot) {
          ...
          const newDirection: LayoutDirection =
            position.x < rootX ? 'left' : 'right'
          const currentDirection =
            metadata.nodeMetadata[nodeId]?.direction ?? 'right'
        ...
        if (item && item.level === 1) {
          ...
          if (parentRoot) {
            ...
            const newDirection: LayoutDirection =
              position.x < rootX ? 'left' : 'right'
            const currentDirection =
              updatedNodeMetadata[id]?.direction ?? 'right'
