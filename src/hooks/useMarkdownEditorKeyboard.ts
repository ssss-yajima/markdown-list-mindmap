    ...
    (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      if (e.key === 'Enter' && !e.shiftKey && !e.ctrlKey && !e.metaKey) {
        ...
        if (cursorInLine < contentStart) {
          ...
          const newValue =
            value.slice(0, lineInfo.lineEnd) + newLine + value.slice(lineInfo.lineEnd);
          onChange(newValue);
          ...
          // カーソル位置で行を分割
          const remainingContent = lineInfo.content.slice(cursorInLine - contentStart);
          ...
          requestAnimationFrame(() => {
            const newPos =
              lineInfo.lineStart + newCurrentLine.length + 1 + lineInfo.indent.length + nextMarker.length + 1;
            textarea.selectionStart = newPos;
            textarea.selectionEnd = newPos;
          });
        }
