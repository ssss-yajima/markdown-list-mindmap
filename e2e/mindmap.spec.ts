...
test.describe('キーボード操作', () => {
  ...
  async function selectNode(page: import('@playwright/test').Page, nodeText: string) {
    const node = page.locator('[role="group"][aria-roledescription="node"]')
  ...
  test('編集中にEnterで下に兄弟ノード追加', async ({ page }) => {
    ...
    const newNodeInput = page.locator('input[type="text"][value="新しいノード"]');
