...
// 重なりがないことを検証するヘルパー関数
function assertNoOverlaps(positions: NodePosition[], nodeHeight = 40, minGap = 10) {
  // X座標でグループ化（50px単位）
  ...
  for (const [, nodes] of groupedByX) {
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      ...
      if (gap < nodeHeight + minGap) {
        overlaps.push(`"${current.text}" と "${next.text}" の間隔が不十分: ${gap}px`);
      }
    }
  }
...
test.describe('Layout Improvement', () => {
  ...
  test('初期レイアウト: 新規マークダウン入力時にノードが重ならない', async ({ page }) => {
...
- ルート2
  ...
  - 子2-2`);
    ...
    for (const [xKey, nodes] of groupedByX) {
      ...
      for (let i = 0; i < sorted.length - 1; i++) {
        ...
        // 十分な間隔があることを確認
        expect(gap, `ノード "${current.text}" と "${next.text}" の間隔`).toBeGreaterThanOrEqual(nodeHeight + minGap);
      }
    }
  });
  ...
  - 子3`);
    ...
    const child1Node = page.locator('.react-flow__node').filter({ hasText: '子1' }).first();
  ...
  - フェーズ2
    ...
    for (const before of positionsBefore) {
      ...
      if (after) {
        ...
        expect(xDiff, `ノード "${before.text}" のX座標差`).toBeLessThanOrEqual(maxPositionDiff);
      }
    }
  });

  test('深いネスト: 各階層のノードが適切な水平位置に配置される', async ({ page }) => {
    const textarea = page.locator('textarea');
...
- もう一つの長い日本語テキストでテストする例文です`);
    ...
    for (const [, nodes] of groupedByX) {
      ...
      for (let i = 0; i < sorted.length - 1; i++) {
        ...
        expect(gap, `ノード "${current.text.substring(0, 20)}..." と "${next.text.substring(0, 20)}..." の間隔`).toBeGreaterThanOrEqual(minGap);
      }
    }
  });
});
...
test.describe('ドラッグ後の衝突解消', () => {
  ...
  test('ドラッグ後の衝突解消: 同一階層のノードと重なる位置に移動すると自動調整される', async ({ page }) => {
    const textarea = page.locator('textarea');
  ...
  - ノードC`);
    ...
    // ノードAを取得
    const nodeA = page.locator('.react-flow__node').filter({ hasText: 'ノードA' }).first();
    const nodeB = page.locator('.react-flow__node').filter({ hasText: 'ノードB' }).first();
  ...
  - 子ノード2`);
    ...
    // ドラッグ前のX座標を取得
    const positionsBefore = await getNodePositions(page);
    const child1Before = positionsBefore.find(n => n.text.includes('子ノード1'));
    expect(child1Before).toBeDefined();

    const child1XBefore = child1Before!.x;

    // 子ノード1を大きく上方向にドラッグ
    const child1Node = page.locator('.react-flow__node').filter({ hasText: '子ノード1' }).first();
